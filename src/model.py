...
class KGEP(object):
    ...
    def _build_model(self, n_user, n_entity, n_relation):
        ...
        users, u_relationns = self.get_neighbors(self.user_indices)

        self.r_interact = tf.nn.embedding_lookup(self.relation_emb_matrix, u_relationns[0])
        self.r_interact = tf.reduce_mean(self.r_interact, axis=1)
        # [batch_size, dim]
        self.item_embeddings_agg, self.aggregators = self.aggregate(entities, relations)
    ...
    def aggregate(self, entities, relations):
        ...
        for i in range(self.n_iter):  # (0->2)
            ...
            for hop in range(self.n_iter - i):  # (0->2)
                #    (0->2) (0->1)
