...
def topk_settings(show_topk, train_data, test_data, n_item):
    if show_topk:
        user_num = 100
        k_list = [10, 20, 30, 40]
        train_record = get_user_record(train_data, True)  # all positive and negative samples
        test_record = get_user_record(test_data, False)   # test set
        user_list = list(set(train_record.keys()) & set(test_record.keys()))  # common user of training set and test set
